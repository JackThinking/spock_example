...
def cutRod(prices, length) {
    if (length == 0) new RevenueDetails(0, []) else {
        ...
        for (rodSize in 1..length) {
            ...
            def potentialRevenue = new RevenueDetails(prices[rodSize] + revenueFromSecondHalf.revenue,
                    revenueFromSecondHalf.splits + rodSize)
            if (potentialRevenue.revenue > maxRevenueDetails.revenue) maxRevenueDetails = potentialRevenue
        }
        maxRevenueDetails
    }
}

// rodPrices是全局变量，length 映射到 desiredLength (看上去有点绕)
